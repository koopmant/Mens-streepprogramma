...
class MainApplication(Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        # Bouw het inlogscherm op
        self.login_scherm()
        
    def login_scherm(self):
        self.pack(side="top", fill="both", expand=True)
        # Invoer van eigen naam
        self.naam = Entry(self, justify=CENTER, font="Calibri, 20", bd=0)
        self.naam.pack(ipady=20, padx=100, pady=50)
        self.naam.bind("<Return>", self.check_name)
        self.naam.focus()
        
        # Informatie ruimte
        self.response = Text(self, font="Calibri, 16", bd=0, width=25, height=4, bg=ACHTERGRONDKLEUR, fg="black")
    ...
    def streep_scherm(self):
        ...
        ML = Image.open('menslogo.png')
        achtergrond = ImageTk.PhotoImage(ML.resize((408,432), Image.ANTIALIAS))
        self.backlabel = Label(self.master, bg=ACHTERGRONDKLEUR, image= achtergrond)
        self.backlabel.image = achtergrond
    ...
    def toggle_fullscreen(self, master=None):
        self.fullscreenstate = not self.fullscreenstate
        self.master.attributes("-fullscreen", self.fullscreenstate)
        
    def exit_fullscreen(self, master=None):
...
if __name__ == "__main__":
    ...
    with open('december 2014.csv', 'rb') as csvfile:
        ...
        root = Tk()
        root.title("Streepsysteem Studievereniging Mens, versie %s" % (VERSIE))
